...
import ModalDetailPribadi from "../../../components/Kalender/ModalDetailPribadi";
import { getKalenderPribadi } from "../../../client/KalenderClient";
...
const PageKalender = () => {
  ...
  const _getKalenderData = async () => {
    try {
      // Ambil data utama (pendidikan, kegiatan, label dsb)
      let query = { tanggal: currentDate };
      const { data } = await getKalenderData(query);

      // --- Handle Data Kalender Pendidikan/Kegiatan ---
      setKalender(data);
      ...
      // --- Label List Untuk Mapping Warna Pribadi ---
      const labels = data?.label || [];

      // --- Kalender Pribadi (Ambil dari API terpisah) ---
      const kalenderPribadiRes = await getKalenderPribadi();
      const kalenderPribadiList = kalenderPribadiRes?.data || [];

      // Mapping event pribadi, warna diambil dari labels/dt.warna (ID Label)
      const eventsDataPribadi = kalenderPribadiList.map((dt) => ({
        ...
        id: dt?.id,
        isPribadiEvent: true,
        color: (labels.find(l => String(l.id) === String(dt.warna))?.warna) || "#bbb", // label dari id warna
        deskripsi: dt?.deskripsi,
        allDay: true,
      }));

      // Gabungkan semua event ke calendarEvents
      setCalendarEvents([
        ...eventsDataKegiatan,
      ...
      // optional: handling error fetching data
      console.error("Error get kalender data:", err);
    }
  };
  ...
  const onClickCalendar = (e) => {
    ...
    else if (ext.isPribadiEvent) modalId = "ModalDetailPribadi";
